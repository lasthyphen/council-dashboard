...
class ConnectorTheGraph {
    ...
    async rolesForAddress(organization, appAddress) {
        return __classPrivateFieldGet(this, _gql).performQueryWithParser(queries.ROLE_BY_APP_ADDRESS('query'), { appAddress: appAddress.toLowerCase() }, (result) => parseRoles(result, organization));
    }
    async permissionsForOrg(organization) {
        return __classPrivateFieldGet(this, _gql).performQueryWithParser(queries.ORGANIZATION_PERMISSIONS('query'), { orgAddress: organization.address.toLowerCase() }, (result) => parsePermissions(result, organization));
    }
    onPermissionsForOrg(organization, callback) {
        return __classPrivateFieldGet(this, _gql).subscribeToQueryWithParser(queries.ORGANIZATION_PERMISSIONS('subscription'), { orgAddress: organization.address.toLowerCase() }, callback, (result) => parsePermissions(result, organization));
    }
    async appByAddress(organization, appAddress) {
        return __classPrivateFieldGet(this, _gql).performQueryWithParser(queries.APP_BY_ADDRESS('query'), { appAddress: appAddress.toLowerCase() }, (result) => parseApp(result, organization));
    }
    ...
    async repoForApp(organization, appAddress) {
        return __classPrivateFieldGet(this, _gql).performQueryWithParser(queries.REPO_BY_APP_ADDRESS('query'), { appAddress: appAddress.toLowerCase() }, (result) => parseRepo(result, organization));
    }
}
_gql = new WeakMap();
